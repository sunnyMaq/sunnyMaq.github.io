...
function tween(obj,json,options){
    ...
    options.type=options.type || Tween.Bounce.easeInOut;

    var iCount=Math.ceil(options.time/(1000/60));
    var start={};
    ...
    obj.timer=setInterval(function(){
        ...
        for(var name in json){
            var cur=options.type(options.time/iCount* n,start[name],json[name]-start[name],options.time);
            if(name=='opacity'){
                obj.style.opacity=cur;
                obj.borderRightStyle.filter='alpha(opacity='+cur*100+')'
            }else {
